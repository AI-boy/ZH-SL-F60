				 ...
				 if((battery_voltage_value>=dianchi_qianya_baohu)&&(discharge_index==0x02))    		//电池电压>欠压保护点
				 {
					...
					{								    
						...
						//---------------------------------------------------------亮灯模式
						 if(light_mode==0x00 && work_mode==0x00)//常亮模式
						 {
						 	if(ho_cnt==0 && mi_cnt<30)
						 	{
								...
								{
								  	PID_CC_discharge.SetPoint = data_cc_var*0.01;
								}
								else
								{
						 	...
						 	else
						 	{
							 	...
							 	else
							 	{
							 		if(voltage_adjust_index==0x05)
									{
									  	PID_CC_discharge.SetPoint = data_cc_var*0.01*0.7;
									}
									else
									{
						 ...
						 else if(light_mode==0x01 && work_mode==0x00)//感应模式
						 {
						 	...
						 	{
						 		PID_CC_discharge.SetPoint = data_cc_var*0.03; //无感应时 5%输出
						 	}
						 }
						 else if(light_mode==0x02 && work_mode==0x00)//智能模式
						 {
						 	if(Adjust_Pout_Flag==0x01 && voltage_adjust_index<0x04) //
						 	{
						 		if(ho_cnt==0 && mi_cnt<30)
							 	{
									if(voltage_adjust_index==0x05)
						...
						//---------------------------------------------------------
						    ...
						    else if(PID_CC_discharge.SetPoint<=1)
						    {
						    	bost_pwm_value = 25;
						    }
